...
int main(void)
{
  ...
  /* Retriving Data from Micro SD Card */
	...
	/*********************************** Extract Developer Public key ***************************************/
			...
			if( memcmp(fwHeader->hash, aFirmwareHashCalc, HASH_LEN) != 0)
			{
				writetoSerial(&huart1, "[❌] Hash computation Failed \r\n"); /* U+274C */
				return HAL_ERROR;
			}

			writetoSerial(&huart1, "[✔] Hash computation verified \r\n"); /* U+2705 */

			status = mbedtls_pk_verify(
					&devCert.pk,
					MBEDTLS_MD_SHA256,
					aFirmwareHashCalc,
					SHA256_LEN,
...
/****** Jumping to Application Software located in CRYPT_APPL_ADDR (0x08080000UL) ************/
	writetoSerial(&huart1, "[INFO] Jumping to Crypto Application .. \r\n");
	JumpToCryptoApplication();
