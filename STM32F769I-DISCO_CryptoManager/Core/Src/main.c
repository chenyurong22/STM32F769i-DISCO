...
volatile uint8_t rxBuffer[UART_TX_LEN];
volatile uint8_t count;

TaskHandle_t DecryptTaskHandle;
...
int main(void)
{
	...
	/* Initializing Crypto Receive Structure */

	writetoSerial(&huart1, "Hello Crypto ..\r\n");
	Init_CryptoRx_Process(&CryptoRx);
...
//	defaultTaskHandle = osThreadCreate(osThread(defaultTask), NULL);
	...
	ASSERT_TASK(status, defaultTaskHandle)

	/* definition and creation of StartDecryptTask */
	status = xTaskCreate(StartDecryptTask,
			"DecryptTaskHandle",
			800,
			&CryptoRx,
			6,
	...
	ASSERT_TASK(status, DecryptTaskHandle);
