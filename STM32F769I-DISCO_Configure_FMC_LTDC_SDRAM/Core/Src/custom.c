...
const char* ErrorString(ERROR_t bError)
{
	...
	case FUN_ERROR_INVALID:
		return "Invalid function";
	case FUN_ERROR_EMPTY:
...
void writeToSerialWithYeild(USART_TypeDef *huart, char *pMsg)
{
	while (*pMsg)
	{
		HAL_UART_Transmit(huart, pMsg, 1, HAL_MAX_DELAY);
		pMsg++;
		//taskYIELD();
	}
}
...
uint16_t dataADC;
uint16_t dataArray[32];

float voltADC;
uint8_t EOCFlagON = 0;
uint8_t DTCFlagON = 0;
volatile uint16_t byteRemaining;
extern uint16_t sensorData[32];


float ADC_CountToVolt(uint16_t countADC)
{
	return (countADC * 3.3) / 4095;
}
