...
uint8_t *pWelcome = "Jai Veer Hanuman \r\n";
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MPU_Config();
	...
	SCB_EnableICache(); /* ✅ Important */
	SCB_EnableDCache(); /* ✅ Important */
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  writetoSerial(&huart1, pWelcome);

  /* Testing MACROS */
  uint16_t registerVal = 0;

  DD_READ_REG(registerVal);
  DD_WRITE_REG(registerVal, 0xDEBA);
  DD_SET_BIT(registerVal, 0);
  DD_CLEAR_BITS(registerVal, 0x00FF);
  DD_MOD_REG(registerVal, 0x00FF, 0x00EF);
  /* Testing MACROS ends */

  /* Writing into SDRAM1 */
  uint16_t *pFramBuff = (uint16_t*)0xC0000000;
  uint32_t pixelIndex = 0;

  while(pixelIndex < 32)
  {
	  pFramBuff[pixelIndex++] = 0xDEBA;
  }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : uSD_CLK_Pin */
  GPIO_InitStruct.Pin = uSD_CLK_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  GPIO_InitStruct.Alternate = GPIO_AF11_SDMMC2;
  HAL_GPIO_Init(uSD_CLK_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pin : EXT_SCL_Pin */
