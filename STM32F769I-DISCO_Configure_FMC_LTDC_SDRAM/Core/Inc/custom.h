...
/* CUSTOM REGISTER LEVEL MACRO STARTS */
#define DD_READ_REG(REG)  (REG)
#define DD_WRITE_REG(REG, VAL)  ((REG) = (VAL))

#define DD_MOD_REG(REG, READ_MASK, VAL) \
	(DD_WRITE_REG(REG, (((DD_READ_REG(REG)) & ~ (READ_MASK)) | ( VAL))))

#define DD_CLEAR_BITS(REG, SETMASK) \
	DD_WRITE_REG(REG, (DD_READ_REG(REG) & ~(SETMASK)))

#define DD_SET_BIT(REG, BIT_POS) \
	DD_WRITE_REG((REG),  (DD_READ_REG(REG) | (1U << (BIT_POS))))

#define DD_TOGGLE_BIT(REG, BIT_POS) \
	DD_WRITE_REG((REG),  (DD_READ_REG(REG) ^ (1U << (BIT_POS))))

#define DD_TOGGLE_BITS(REG, TOGGLEMASK) \
	DD_WRITE_REG((REG),  (DD_READ_REG(REG) ^ (TOGGLEMASK)))

/* CUSTOM REGISTER LEVEL MACRO ENDS */
...
void writeToSerialWithYeild(USART_TypeDef *huart, char *pMsg);
...
/*ADC1 specific APIs */
void ADC1_Init();
float ADC_CountToVolt(uint16_t countADC);
