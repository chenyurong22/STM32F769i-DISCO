...
#define ALIGN_DOWN(addr)   ((addr) & ~(CACHE_LINE_SIZE - 1))
#define ALIGN_UP(addr)     (((addr) + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1))

/* USER CODE END Private defines */

static void SD_CleanDCache(uint8_t *buff, uint32_t size)
{
    uint32_t addr = (uint32_t)buff;
    uint32_t aligned_addr = ALIGN_DOWN(addr);
    uint32_t aligned_size = ALIGN_UP(size + (addr - aligned_addr));

    SCB_CleanDCache_by_Addr((uint32_t*)aligned_addr, aligned_size);
}

static void SD_InvalidateDCache(uint8_t *buff, uint32_t size)
{
    uint32_t addr = (uint32_t)buff;
    uint32_t aligned_addr = ALIGN_DOWN(addr);
    uint32_t aligned_size = ALIGN_UP(size + (addr - aligned_addr));
